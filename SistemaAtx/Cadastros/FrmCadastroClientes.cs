    ...
    public partial class FrmCadastroClientes : Form
    {
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.AbrirCon();
                sql = "UPDATE clientes SET nome = @nome, telefone = @telefone, celular = @celular, cpf = @cpf, endereco = @endereco, bairro = @bairro, cidadeuf = @cidadeuf, email = @email where id = @id";
                cmd = new MySqlCommand(sql, con.con);
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.AbrirCon();
                sql = "INSERT INTO clientes (nome, telefone, celular, cpf, endereco, bairro, cidadeuf, email) VALUES (@nome, @telefone, @celular, @cpf, @endereco, @bairro, @cidadeuf, @email)";
                cmd = new MySqlCommand(sql, con.con);
                cmd.Parameters.AddWithValue("@nome", txtNome.Text);
                cmd.Parameters.AddWithValue("@telefone", txtTelefone.Text);
                cmd.Parameters.AddWithValue("@celular", txtCelular.Text);
                ...
                MessageBox.Show("Cliente Salvo com Sucesso!", "Registros Salvos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                btnNovo.Enabled = true;
                btnSalvar.Enabled = false;
                limparCampos();
                desabilitarCampos();
        ...
        {
            var resultado = MessageBox.Show("Deseja Realmente Excluir o Cliente?", "Excluir Registro", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            try
            {
                if (resultado == DialogResult.Yes)
                {
                    ...
                    MessageBox.Show("Cliente Excluido com Sucesso!", "Registro Excluido", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    btnNovo.Enabled = true;
                    btnEditar.Enabled = false;
                    btnExcluir.Enabled = false;
                    limparCampos();
